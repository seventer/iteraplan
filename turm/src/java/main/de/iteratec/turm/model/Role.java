...
/**
 * This class can hold Role data. It is mainly used for displaying the role data
 * on a JSP.
 * 
 * TODO: check if this can be merged with the {@link RoleContainer} class.
 */
public class Role implements IdEntity {
  ...
  public Role(Number id, Number olVersion, String roleName, String description, List<User> users) {
    super();
    this.id = id;
  ...
  public String[] getUserIdsAsStringArray() {
    String[] result = new String[getUsers().size()];
    int index = 0;
    for (Iterator<User> it = getUsers().iterator(); it.hasNext(); index++) {
      User user = it.next();
      result[index] = user.getId().toString();
    }
    return result;
  }
