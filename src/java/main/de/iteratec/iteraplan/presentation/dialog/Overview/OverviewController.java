 ...
 */
package de.iteratec.iteraplan.presentation.dialog.Overview;
...
public class OverviewController extends GuiController {
  ...
  public void init(ModelMap model, HttpSession session, HttpServletRequest request) {
    ...
    UserContext.getCurrentUserContext().getPerms().assureFunctionalPermission(TypeOfFunctionalPermission.OVERVIEW);

    GuiContext context = GuiContext.getCurrentGuiContext();

    OverviewDialogMemory overviewDialogMemory;
    if (context.hasDialogMemory(getDialogName())) {
      overviewDialogMemory = (OverviewDialogMemory) context.getDialogMemory(getDialogName());
    }
    else {
      overviewDialogMemory = new OverviewDialogMemory();
    }

    overviewDialogMemory.setOverviewLists(overviewService.getElementLists());

    model.addAttribute("dialogMemory", overviewDialogMemory);
    //set dialogue memory to model and update GUI context
    updateGuiContext(overviewDialogMemory);
  }

  @Override
  protected String getDialogName() {
    return Dialog.OVERVIEW.getDialogName();
  }
