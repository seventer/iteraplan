 ...
 *
 */
public class BusinessProcessMappingStrategy implements BusinessMappingStrategy {
  ...
  private boolean hasTopLevelElementsOnly(BusinessMapping bm) {
    ...
    return BusinessUnit.TOP_LEVEL_NAME.equals(buName) && Product.TOP_LEVEL_NAME.equals(productName) && isr == null;
  }

  /** {@inheritDoc} */
  public void validate(BusinessMappingItems bmi) {
    if (bmi.getBusinessUnits().isEmpty() || bmi.getInformationSystemReleases().isEmpty() || bmi.getProducts().isEmpty()) {
  ...
  public <T extends BuildingBlock> void addOwningEntity(BusinessMapping businessMapping, T source) {
    if (source.getId() != null) {
      ...
      businessMapping.addBusinessProcess(SpringServiceFactory.getBusinessProcessService().loadObjectById(source.getId()));
    }
    else {
      businessMapping.addBusinessProcess((BusinessProcess) source);
    }
  }
  ...
  public boolean equalsIds(BusinessMapping existingBm, BusinessMapping newBm) {
    ...
    boolean isAnyParameterNull = bu.getId() == null || isr.getId() == null || product.getId() == null;
    boolean isAnyReferenceNull = existingBm.getBusinessUnit() == null || existingBm.getInformationSystemRelease() == null || existingBm.getProduct() == null;
    
    if (isAnyParameterNull || isAnyReferenceNull) {
      return false;
    }
    
    return bu.getId().equals(existingBm.getBusinessUnit().getId()) && isr.getId().equals(existingBm.getInformationSystemRelease().getId()) && product.getId().equals(existingBm.getProduct().getId());
  }
}
