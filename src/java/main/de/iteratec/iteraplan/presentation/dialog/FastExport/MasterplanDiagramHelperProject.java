...
public class MasterplanDiagramHelperProject extends MasterplanDiagramHelper {
  ...
  public List<BuildingBlock> determineResults(BuildingBlock start, DiagramVariant variant, MasterplanOptionsBean bean) {
    ...
    Project proj = (Project) start;
    initialList.add(proj);

    if (bean != null) {
      bean.setSelectedBbType(Constants.BB_PROJECT);
      MasterplanRowTypeOptions levelOOptions = new MasterplanRowTypeOptions("", Constants.BB_PROJECT, 0, new ArrayList<DateInterval>(),
      ...
      levelOOptions.getColorOptions().setDimensionAttributeId(Integer.valueOf(GraphicalExportBaseOptions.NOTHING_SELECTED));
