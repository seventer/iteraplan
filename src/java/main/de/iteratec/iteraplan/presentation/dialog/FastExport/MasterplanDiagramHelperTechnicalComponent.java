...
public class MasterplanDiagramHelperTechnicalComponent extends MasterplanDiagramHelper {
  ...
  public List<BuildingBlock> determineResults(BuildingBlock start, DiagramVariant variant, MasterplanOptionsBean bean) {
    ...
    TechnicalComponentRelease tcr = (TechnicalComponentRelease) start;
    initialList.add(tcr);

    if (bean != null) {
      bean.setSelectedBbType(Constants.BB_TECHNICALCOMPONENTRELEASE);

      MasterplanRowTypeOptions levelOOptions = new MasterplanRowTypeOptions("", Constants.BB_TECHNICALCOMPONENTRELEASE, 0,
          new ArrayList<DateInterval>(), new ArrayList<BBAttribute>(), new ArrayList<ColumnEntry>());
      levelOOptions.getColorOptions().setAvailableColors(SpringGuiFactory.getInstance().getMasterplanColors());
      levelOOptions.getColorOptions().setDimensionAttributeId(Integer.valueOf(GraphicalExportBaseOptions.STATUS_SELECTED));
      levelOOptions.getColorOptions().setColorRangeAvailable(false);
      levelOOptions.getColorOptions().refreshDimensionOptions(
          StringEnumReflectionHelper.getLanguageSpecificEnumValues(TypeOfStatus.class, UserContext.getCurrentLocale()));

      bean.setLevel0Options(levelOOptions);
    }
