 ...
 * @author est
 */
public class LandscapeDiagramHelperInformationSystemTcr extends LandscapeDiagramHelper<InformationSystemRelease> {

  /**
   * @param startElement
   *          The information system release to start from.
   */
  public LandscapeDiagramHelperInformationSystemTcr(InformationSystemRelease startElement) {
  ...
  protected List<? extends BuildingBlock> filterRowElements() {
    ...
    if (TypeOfBuildingBlock.TECHNICALCOMPONENTRELEASE.equals(getContentTypeOfBb())) {
      Set<TechnicalComponentRelease> tcrs = getStartElement().getTechnicalComponentReleases();

      if (!tcrs.isEmpty()) {
        for (TechnicalComponentRelease tcr : tcrs) {
          filteredList.addAll(tcr.getArchitecturalDomains());
        }
        return filteredList;
      }
      else {
