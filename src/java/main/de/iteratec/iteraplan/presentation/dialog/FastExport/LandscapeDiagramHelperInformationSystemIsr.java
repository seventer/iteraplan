...
/**
 * Helper for creating a fast export landscape diagram from an information system release (start
 * element). Content type: information system releases. Column type: business processes. Row type:
 * business units.
 * 
 * @author est
 */
public class LandscapeDiagramHelperInformationSystemIsr extends LandscapeDiagramHelper<InformationSystemRelease> {

  /**
   * @param startElement
   *          The information system release to start from.
   */
  public LandscapeDiagramHelperInformationSystemIsr(InformationSystemRelease startElement, TypeOfBuildingBlock columnType, TypeOfBuildingBlock rowType) {
  ...
  protected List<BuildingBlock> filterRowElements() {
    ...
    for (BuildingBlock rowElement : getRowElements()) { // Loop through all row Elements (ex. BU)
      ...
      // Projects don't have Business Mappings
      if (getRowTypeOfBb().equals(TypeOfBuildingBlock.PROJECT)) {
        Project curProject = (Project) rowElement;
        // Include only the Projects who have the startElement
        if (curProject.getInformationSystemReleases().contains(getStartElement())) {
