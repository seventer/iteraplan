 ...
 *       being made by the JSP (see ManyAssociationSetComponentComboboxView.jsp)
 */
public abstract class ManyAssociationSetComponentModelDL<F extends IdEntity, T extends Comparable<? super T> & Entity> extends
    ManyAssociationSetComponentModel<F, T> {

  private static final long serialVersionUID = -4544687124910003598L;

  public ManyAssociationSetComponentModelDL(ComponentMode componentMode, String htmlId, String tableHeaderKey, String[] columnHeaderKeys,
  ...
  @Override
  protected List<T> getAvailableElements(Integer id, List<T> connected) {
  ...
  protected void processElementIdToAdd() {
    if (getElementIdToAdd() != null && getElementIdToAdd().intValue() > 0) {
      T bf = getService().loadObjectById(getElementIdToAdd());
      if (!getConnectedElements().contains(bf)) {
        this.getConnectedElements().add(bf);
      }
    }
  }
  ...
  protected void processElementIdsToAdd() {
    if (getElementIdsToAdd() != null) {
      for (Integer id : getElementIdsToAdd()) {
        if (id.intValue() > 0) {
          T bf = getService().loadObjectById(id);
          if (!getConnectedElements().contains(bf)) {
            this.getConnectedElements().add(bf);
          }
        }
