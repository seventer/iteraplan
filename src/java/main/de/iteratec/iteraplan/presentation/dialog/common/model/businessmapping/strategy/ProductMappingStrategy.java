 ...
 *
 */
public class ProductMappingStrategy implements BusinessMappingStrategy {
  ...
  private boolean hasTopLevelElementsOnly(BusinessMapping bm) {
    ...
    return BusinessUnit.TOP_LEVEL_NAME.equals(buName) && BusinessProcess.TOP_LEVEL_NAME.equals(bpName) && isr == null;
  }

  /** {@inheritDoc} */
  public void validate(BusinessMappingItems bmi) {
    if (bmi.getBusinessUnits().isEmpty() || bmi.getInformationSystemReleases().isEmpty() || bmi.getBusinessProcesses().isEmpty()) {
  ...
  public <T extends BuildingBlock> void addOwningEntity(BusinessMapping businessMapping, T source) {
    if (source.getId() != null) {
      ...
      businessMapping.addProduct(SpringServiceFactory.getProductService().loadObjectById(source.getId()));
    }
    else {
      businessMapping.addProduct((Product) source);
    }
  }
  ...
  public boolean equalsIds(BusinessMapping existingBm, BusinessMapping newBm) {
    ...
    boolean isAnyParameterNull = bu.getId() == null || isr.getId() == null || bu.getId() == null;
    boolean isAnyReferenceNull = existingBm.getBusinessUnit() == null || existingBm.getInformationSystemRelease() == null || existingBm.getBusinessProcess() == null;
    
    if (isAnyParameterNull || isAnyReferenceNull) {
      return false;
    }
    
    return bu.getId().equals(existingBm.getBusinessUnit().getId()) && isr.getId().equals(existingBm.getInformationSystemRelease().getId()) && bp.getId().equals(existingBm.getBusinessProcess().getId());
  }
}
