...
public class BusinessDomainModelBuilder extends BuildingBlockModelBuilder {

  private BusinessDomainChangeset changeSet;

  public BusinessDomainModelBuilder(BuildingBlock bb, HistoryBBChangeset changeSet, String applicationBaseUri) {
    super(bb, applicationBaseUri, changeSet);
    if (changeSet != null && ! (changeSet instanceof BusinessDomainChangeset)) {
      throw new IllegalArgumentException("expecting an BusinessDomainChangeSet");
    }

    this.changeSet = (BusinessDomainChangeset) changeSet;
  }
  ...
  public EmailModel createModel() {
    ...
    this.addChange(changes, "Business unit", EmailModel.ADDED, changeSet.getBusinessUnitsAdded());
    this.addChange(changes, "Business unit", EmailModel.REMOVED, changeSet.getBusinessUnitsRemoved());
    this.addChange(changes, "Business process", EmailModel.ADDED, changeSet.getBusinessProcessesAdded());
    this.addChange(changes, "Business process", EmailModel.REMOVED, changeSet.getBusinessProcessesRemoved());
    ...
    this.addChange(changes, "Product", EmailModel.ADDED, changeSet.getProductsAdded());
    this.addChange(changes, "Product", EmailModel.REMOVED, changeSet.getProductsRemoved());
