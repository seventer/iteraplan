...
public class JsonModelRepresentationHandler implements RepresentationHandler {
  /**{@inheritDoc}**/
  public Representation process(Request request, Response response, Map<String, Object> arguments) {
    JsonArray json = new JsonModelMapper(ElasticMiContext.getCurrentContext().getContextMetamodel(), request.getRootRef().toString())
        .write(ElasticMiContext.getCurrentContext().getContextModel());

    Representation resultRepresentation = new JsonRepresentation(RestUtils.formatToJson(json));
  ...
  public boolean supports(ResourceType resourceType) {
    return ResourceType.MODEL.equals(resourceType);
  }
