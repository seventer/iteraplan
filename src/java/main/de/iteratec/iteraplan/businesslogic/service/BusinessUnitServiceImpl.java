...
/**
 * Implementation of the service interface {@link BusinessUnitService}.
 */
public class BusinessUnitServiceImpl extends AbstractHierarchicalBuildingBlockService<BusinessUnit> implements BusinessUnitService {

  private BusinessUnitDAO        businessUnitDAO;
  private BusinessMappingService businessMappingService;

  @Override
  protected void checkDelete(BusinessUnit buildingBlock) {
    super.checkDelete(buildingBlock);
    checkDeletePermissionsCurrentPerms(buildingBlock);
  }

  @Override
  protected void onAfterDelete(BusinessUnit buildingBlock) {
  ...
  /** {@inheritDoc} */
  @Override
  protected DAOTemplate<BusinessUnit, Integer> getDao() {
    return businessUnitDAO;
  }

  /**
   * Checks, if the given business unit contains duplicate business mappings.
   * 
   * @param businessUnit The business unit to check.
   * @throws IteraplanBusinessException If the business unit contains duplicate business mappings.
   */
  private void checkForDuplicateBusinessMapping(BusinessUnit businessUnit) {
    final Set<String> set = Sets.newHashSet();
    StringBuilder sb = new StringBuilder(20);

    for (BusinessMapping mapping : businessUnit.getBusinessMappings()) {
  ...
  /** {@inheritDoc} */
  @Override
  public BusinessUnit saveOrUpdate(BusinessUnit entity, boolean cleanup) {
    ...
    BusinessUnit savedBusinessUnit = super.saveOrUpdate(entity, cleanup);
