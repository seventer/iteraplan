...
/**
 * This consistency check checks if there are {@link TechnicalComponentRelease}s that have their
 * status set to UNDEFINED.
 */
public class CheckTcReleasesWithStatusUndefined extends AbstractConsistencyCheck {

  public CheckTcReleasesWithStatusUndefined() {
    super("check_technical_tcrStatusUndefined", Domain.TECHNICAL);
  }

  @Override
  public Result executeCheck(Map<String, Parameter> parameters) {

    List<TechnicalComponentRelease> list = getConsistencyCheckDAO().getTcReleasesWithStatusUndefined();

    for (TechnicalComponentRelease release : list) {
