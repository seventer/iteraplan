 ...
 * that are currently productive (according to the start and end date), but 
 * have their status set to anything other than CURRENT.
 */
public class CheckInformationSystemReleasesActiveWithoutStatusCurrent extends AbstractConsistencyCheck {
  public CheckInformationSystemReleasesActiveWithoutStatusCurrent() {
    super("check_it_isrActiveWithoutStatusCurrent", Domain.IT);
  }

  @Override
  public Result executeCheck(Map<String, Parameter> parameters) {
    List<InformationSystemRelease> list = getConsistencyCheckDAO().getInformationSystemReleasesActiveWithoutStatusCurrent();
    for (InformationSystemRelease release : list) {
