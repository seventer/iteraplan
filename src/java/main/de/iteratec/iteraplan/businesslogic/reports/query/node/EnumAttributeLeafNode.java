...
/**
 * Leaf node for enumeration attribute types.
 */
public final class EnumAttributeLeafNode extends AttributeLeafNode {

  private static final Logger LOGGER = Logger.getIteraplanLogger(EnumAttributeLeafNode.class);

  /**
   * Factory method for creating an instance of this class.
   * 
   * @param resultType The result tpye of the query.
   ...
   * @return An instance of {@code EnumAttributeLeafNode}.
   ...
   */
  public static EnumAttributeLeafNode createNode(Type<?> resultType, Extension extension, int id, Comparator comparator, String pattern) {
    LOGGER.debug("Creating a leaf node for an enumeration attribute type.");
    ...
    return new EnumAttributeLeafNode(resultType, extension, id, comparator, pattern);
  }

  private EnumAttributeLeafNode(Type<?> resultType, Extension extension, int id, Comparator comparator, Object pattern) {
  ...
  protected Criterion getCriterionForComparator() {
    ...
    switch (getComparator()) {
      case LIKE:
        criterion = new IteraplanLikeExpression("name", (String) getProcessedPattern(), true);
        break;
      case EQ:
        criterion = Restrictions.eq("name", getProcessedPattern());
        break;
      case ANY_ASSIGNMENT:
  ...
  DetachedCriteria getWhereCriteria(DetachedCriteria criteria) {
    if (getComparator() == Comparator.NOT_LIKE) {
      Criterion criterion = new IteraplanLikeExpression("numberAV.name", (String) getProcessedPattern(), true);
