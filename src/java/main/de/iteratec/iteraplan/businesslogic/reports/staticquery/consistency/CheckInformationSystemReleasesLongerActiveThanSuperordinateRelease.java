 ...
 * that have a longer productivity lifespan than their superordinate release.
 */
public class CheckInformationSystemReleasesLongerActiveThanSuperordinateRelease extends AbstractConsistencyCheck {

  public CheckInformationSystemReleasesLongerActiveThanSuperordinateRelease() {
    super("check_it_isrLongerActiveThanSuperordinateRelease", Domain.IT);
  }

  @Override
  public Result executeCheck(Map<String, Parameter> parameters) {

    List<InformationSystemRelease> releases = getConsistencyCheckDAO().getInformationSystemReleasesWithParents();

    for (InformationSystemRelease release : releases) {
      if (!release.runtimeWithinPeriod(release.getParent().getRuntimePeriod())) {

        List<Object> row = new ArrayList<Object>();
        row.add(release);
        row.add(release.runtimeStartsAt());
        row.add(release.runtimeEndsAt());
        row.add(release.getParent());
        row.add(release.getParent().runtimeStartsAt());
        row.add(release.getParent().runtimeEndsAt());
  ...
  protected List<ResultColumn> configureColumns() {
    ...
    columns
        .add(new ResultColumn("check.column.informationsystemrelease.parent", DataType.OBJECT, "nonHierarchicalName", Boolean.TRUE, Boolean.FALSE));
    columns.add(new ResultColumn(Constants.TIMESPAN_PRODUCTIVE_FROM, DataType.DATE));
    columns.add(new ResultColumn(Constants.TIMESPAN_PRODUCTIVE_TO, DataType.DATE));
