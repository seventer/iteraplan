...
/**
 * This consistency check checks if there are {@link TechnicalComponentRelease}s that are currently
 * productive (according to the start and end date), but have their status set to anything other
 * than CURRENT.
 */
public class CheckTcReleasesActiveWithoutStatusCurrent extends AbstractConsistencyCheck {

  public CheckTcReleasesActiveWithoutStatusCurrent() {
    super("check_technical_tcrActiveWithoutStatusCurrent", Domain.TECHNICAL);
  }

  @Override
  public Result executeCheck(Map<String, Parameter> parameters) {

    List<TechnicalComponentRelease> releases = getConsistencyCheckDAO().getTcReleasesActiveWithoutStatusCurrent();
