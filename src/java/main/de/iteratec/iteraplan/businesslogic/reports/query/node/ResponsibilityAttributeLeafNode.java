...
/**
 * Leaf node for responsibility attribute types.
 */
public final class ResponsibilityAttributeLeafNode extends AttributeLeafNode {

  private static final Logger LOGGER = Logger.getIteraplanLogger(ResponsibilityAttributeLeafNode.class);
   ...
   * @return An instance of {@code ResponsibilityAttributeLeafNode}.
   * @throws IteraplanBusinessException
   *           If no value was provided and the comparator is not equal to
   *           {@link Comparator#NO_ASSIGNMENT} or {@link Comparator#ANY_ASSIGNMENT}.
   */
  public static ResponsibilityAttributeLeafNode createNode(Type<?> resultType, Extension extension, int id, Comparator comparator, String pattern) {
    LOGGER.debug("Creating a leaf node for a responsibility attribute type.");
    ...
    return new ResponsibilityAttributeLeafNode(resultType, extension, id, comparator, pattern);
  }

  private ResponsibilityAttributeLeafNode(Type<?> resultType, Extension extension, int id, Comparator comparator, Object pattern) {
  ...
  private Criterion createUserCriterion() {
    ...
    return Restrictions.or(Subqueries.propertyIn("userEntity", user), Subqueries.propertyIn("userEntity", userGroup));
  }
  ...
  DetachedCriteria getWhereCriteria(DetachedCriteria criteria) {
    if (getComparator() == Comparator.NOT_LIKE) {
      final Criterion criterion = createUserCriterion();
