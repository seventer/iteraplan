 ...
 */
package de.iteratec.iteraplan.businesslogic.reports.query.options.GraphicalReporting.Line;
 ...
 *
 */
public class LineOptionsBean extends GraphicalExportBaseOptions implements ILineOptionsBean, Serializable {
  ...
  private boolean           timeseriesAttributesActive;

  public LineOptionsBean() {
    super();
    // set end date to today
    Date date = new Date();
    endDateString = DateUtils.formatAsString(date, UserContext.getCurrentLocale());
    // set start date to today minus one year
    Calendar startDateC = Calendar.getInstance();
    startDateC.setTime(date);
    startDateC.roll(Calendar.YEAR, false);
    startDateString = DateUtils.formatAsString(startDateC.getTime(), UserContext.getCurrentLocale());
    getColorOptionsBean().setAvailableColors(SpringGuiFactory.getInstance().getLineColors());
    super.setAvailableGraphicFormats(ExportOption.getGraphicalExportOptions(false));
    super.setSelectedGraphicFormat(Constants.REPORTS_EXPORT_GRAPHICAL_SVG);
  }
  ...
  /**
   * @return timeseriesAttributesActive the timeseriesAttributesActive
   */
  public boolean isTimeseriesAttributesActive() {
    return timeseriesAttributesActive;
  }

  public void setTimeseriesAttributesActive(boolean timeseriesAttributesActive) {
    this.timeseriesAttributesActive = timeseriesAttributesActive;
  }
}
