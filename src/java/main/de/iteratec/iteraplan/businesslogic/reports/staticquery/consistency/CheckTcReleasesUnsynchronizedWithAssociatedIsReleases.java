...
/**
 * This consistency check checks if there are {@link TechnicalComponentRelease}s that are
 * associated with an {@link InformationSystemRelease}, but are not productive throughout the productivity time
 * span of this {@link InformationSystemRelease}.
 */
public class CheckTcReleasesUnsynchronizedWithAssociatedIsReleases extends AbstractConsistencyCheck {

  public CheckTcReleasesUnsynchronizedWithAssociatedIsReleases() {
    super("check_technical_tcrUnsynchronizedWithAssociatedInformationSystemReleases", Domain.TECHNICAL);
  }

  @Override
  public Result executeCheck(Map<String, Parameter> parameters) {

    List<Object[]> listObjects = getConsistencyCheckDAO().getUnsynchronizedTcAndIsReleases();

    for (Object[] objects : listObjects) {
      TechnicalComponentRelease tcr = (TechnicalComponentRelease) objects[0];
      InformationSystemRelease isRelease = (InformationSystemRelease) objects[1];

      if (!isRelease.runtimeWithinPeriod(tcr.getRuntimePeriod())) {

        List<Object> row = new ArrayList<Object>();
        row.add(tcr);
        row.add(tcr.runtimeStartsAt());
        row.add(tcr.runtimeEndsAt());
        row.add(isRelease);
        row.add(isRelease.runtimeStartsAt());
        row.add(isRelease.runtimeEndsAt());
  ...
  protected List<ResultColumn> configureColumns() {
    ...
    columns.add(new ResultColumn(Constants.BB_INFORMATIONSYSTEMRELEASE, DataType.OBJECT, "identityString", Boolean.TRUE, Boolean.FALSE));
    columns.add(new ResultColumn(Constants.TIMESPAN_PRODUCTIVE_FROM, DataType.DATE));
    columns.add(new ResultColumn(Constants.TIMESPAN_PRODUCTIVE_TO, DataType.DATE));
