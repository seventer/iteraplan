...
/**
 * This class represents an excel sheet with contents for {@link BusinessUnit}s.
 */
public class BusinessUnitExcelSheet extends CommonSheetContent<BusinessUnit> {

  private static final String SHEET_KEY = Constants.BB_BUSINESSUNIT_PLURAL;
   ...
   */
  public BusinessUnitExcelSheet(Set<BusinessUnit> contents, ExportWorkbook context, List<AttributeType> activatedAttributeTypes,
  ...
  protected void addSpecificHeaders(List<Header> headers) {
    ...
    String coverageHeaderNameExtended = bm + UNIT_OPENER + process + UNIT_SEPARATOR + product + UNIT_SEPARATOR + isr + UNIT_CLOSER;
   ...
   */
  @Override
  protected void addSpecificLineContents(BusinessUnit bu, List<Object> lineContents) {
    if (bu == null) {
      return;
    }
    lineContents.add(ExcelHelper.concatMultipleHierarchicalNames(bu.getBusinessDomains(), IN_LINE_SEPARATOR));

    Set<BusinessProcess> bpSet = new HashSet<BusinessProcess>();
    Set<Product> productSet = new HashSet<Product>();
    Set<InformationSystemRelease> isrSet = new HashSet<InformationSystemRelease>();

    ExcelHelper.extractContentsFromBMs(bu.getBusinessMappings(), bpSet, null, isrSet, productSet);
    lineContents.add(ExcelHelper.concatMultipleHierarchicalNames(bpSet, IN_LINE_SEPARATOR));
    lineContents.add(ExcelHelper.concatMultipleHierarchicalNames(productSet, IN_LINE_SEPARATOR));
    lineContents.add(ExcelHelper.concatBPandProdandISR(bu.getBusinessMappings(), UNIT_OPENER, UNIT_SEPARATOR, UNIT_CLOSER, IN_LINE_SEPARATOR));
