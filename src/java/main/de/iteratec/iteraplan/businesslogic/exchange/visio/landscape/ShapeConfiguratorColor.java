...
/**
 * Assigns color to a shape based on the attribute values of a building block.
 * 
 * @see de.iteratec.iteraplan.businesslogic.exchange.visio.landscape.ShapeConfigurator
 */
public class ShapeConfiguratorColor extends ShapeConfigurator<ColorDimension> {

  /**
   * Creates a new shape configurator.
   */
  public ShapeConfiguratorColor(ColorDimension dimension) {
    super(dimension);
  }

  @Override
  public <T extends BuildingBlock> void configureShape(Shape shape, T bb) {
    Color color = getDimension().getValue(bb);
  ...
  public void configureShape(Shape shape, String attributeValue) {
    Color color = getDimension().getValue(attributeValue);
    shape.setFillForegroundColor(color);
  }
}
