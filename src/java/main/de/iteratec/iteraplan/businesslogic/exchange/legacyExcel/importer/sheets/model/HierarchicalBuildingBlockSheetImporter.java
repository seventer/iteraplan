...
public abstract class HierarchicalBuildingBlockSheetImporter<T extends AbstractHierarchicalEntity<T>> extends SheetImporter<T> {

  public HierarchicalBuildingBlockSheetImporter(Locale locale, TypeOfBuildingBlock tob, ProcessingLog processingLog) {
  ...
  protected boolean importRowIntoBB(LandscapeRowData rowData, LandscapeData landscapeData) {
    ...
    final Integer id = getId(buildingBlockRowData);
    Cell descriptionCell = getDescriptionCell(buildingBlockRowData);

    // each entity needs a unique name
    String name = getName(buildingBlockRowData);
    Cell nameCell = rowData.getBuildingBlocks().get(getConstant(getTypeOfBuildingBlockOnSheet().getPluralValue()));

    T entity = createOrLoad(buildingBlockRowData);
    ...
    landscapeData.addAttributes(entity, rowData.getAttributes());

    Map<String, Cell> contContent = rowData.getBuildingBlocks();
    Map<String, Cell> emptyAttributes = Maps.newHashMap();
    landscapeData.addRelation(entity, contContent, emptyAttributes);
  ...
  protected boolean isValidRow(Map<String, Cell> buildingBlockRowData) {
    ...
    // hierarchical entities may not contain ":" characters
    if (!isNameValid(name)) {
