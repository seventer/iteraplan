...
/**
 * This class is responsible for creation of an excel report for {@link BusinessObject}s.
 */
public class BusinessObjectExcelReport extends ExcelReport {

  private Set<BusinessObject> boList;
   ...
   * @param boList
   *          the {@link BusinessObject}s for the report
   * @param queryData
   ...
   */
  public BusinessObjectExcelReport(ExportWorkbook context, BuildingBlockTypeDAO bbtDao, AttributeTypeDAO attributeTypeDAO,
      Set<BusinessObject> boList, ExcelAdditionalQueryData queryData, String serverURL) {
    this.init(context, bbtDao, attributeTypeDAO, queryData, TypeOfBuildingBlock.BUSINESSOBJECT, serverURL);
    this.boList = boList;
  }
  ...
  public ExportWorkbook createReport() {
    ...
    for (BusinessObject bo : this.boList) {
      allISRs.addAll(bo.getInformationSystemReleases());
      allISIs.addAll(bo.getInformationSystemInterfaces());
      allBFs.addAll(bo.getBusinessFunctions());
      allBDs.addAll(bo.getBusinessDomains());
    }
