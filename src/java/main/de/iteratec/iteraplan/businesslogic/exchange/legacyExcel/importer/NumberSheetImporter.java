...
/**
 * Concrete sheet importer for number attributes.
 * @author sip
 */
public class NumberSheetImporter extends AttributeSheetImporter<NumberAttData> {
   ...
   */
  public NumberSheetImporter(Locale locale, ProcessingLog processingLog) {
  ...
  public void readContentFromSheet(List<Row> sheetContentRows, Map<String, Integer> headline, AttributeData attributeData) {
    ...
    List<NumberAttData> result = Lists.newArrayList();
    int currentRow = 0;
    while (currentRow < sheetContentRows.size()) {
      ...
      CellValueHolder lowerbound = new CellValueHolder(initialRowContent.get(lowerboundColumnName));
      CellValueHolder upperbound = new CellValueHolder(initialRowContent.get(upperboundColumnName));
      CellValueHolder unit = new CellValueHolder(initialRowContent.get(unitColumnName));
      CellValueHolder uniform = new CellValueHolder(initialRowContent.get(uniformColumnName));
      CellValueHolder activeforbb = new CellValueHolder(initialRowContent.get(activeforbbColumnName));
      CellValueHolder userRanges = new CellValueHolder(initialRowContent.get(userRangesColumnName));
      ...
      if(!isBooleanValue(uniform)) {
        getProcessingLog().warn("[{0}] Mandatory value for attribute {1} doesn\'t match yes/no format.", uniform.getCellRef(), name.getAttributeValue());
      }
      
      currentRow++;

      result.add(new NumberAttData(name, oldName, description, groupstr, mandatory, activeforbb, lowerbound, upperbound, unit, uniform, userRanges));
    }

    attributeData.setNumberData(result);
  }
