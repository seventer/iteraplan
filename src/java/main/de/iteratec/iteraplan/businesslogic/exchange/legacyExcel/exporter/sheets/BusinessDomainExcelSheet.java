...
/**
 * This class represents an excel sheet with contents for {@link BusinessDomain}s.
 */
public class BusinessDomainExcelSheet extends CommonSheetContent<BusinessDomain> {

  private static final String SHEET_KEY = Constants.BB_BUSINESSDOMAIN_PLURAL;
   ...
   */
  public BusinessDomainExcelSheet(Set<BusinessDomain> contents, ExportWorkbook context, List<AttributeType> activatedAttributeTypes,
   ...
   */
  @Override
  protected void addSpecificLineContents(BusinessDomain bd, List<Object> lineContents) {
    if (bd != null) {
      lineContents.add(ExcelHelper.concatMultipleHierarchicalNames(bd.getBusinessProcesses(), IN_LINE_SEPARATOR));
      lineContents.add(ExcelHelper.concatMultipleHierarchicalNames(bd.getBusinessFunctions(), IN_LINE_SEPARATOR));
      lineContents.add(ExcelHelper.concatMultipleHierarchicalNames(bd.getProducts(), IN_LINE_SEPARATOR));
      lineContents.add(ExcelHelper.concatMultipleHierarchicalNames(bd.getBusinessUnits(), IN_LINE_SEPARATOR));
      lineContents.add(ExcelHelper.concatMultipleHierarchicalNames(bd.getBusinessObjects(), IN_LINE_SEPARATOR));
    }
  }
}
