...
/**
 * This class is responsible for creation of an excel report for {@link ArchitecturalDomain}s.
 */
public class ArchitecturalDomainExcelReport extends ExcelReport {

  private Set<ArchitecturalDomain> adList;
   ...
   * @param adList
   *          the {@link ArchitecturalDomain}s for the report
   * @param queryData
   ...
   */
  public ArchitecturalDomainExcelReport(ExportWorkbook context, BuildingBlockTypeDAO bbtDao, AttributeTypeDAO attributeTypeDAO,
      Set<ArchitecturalDomain> adList, ExcelAdditionalQueryData queryData, String serverURL) throws IteraplanBusinessException {
    this.init(context, bbtDao, attributeTypeDAO, queryData, TypeOfBuildingBlock.ARCHITECTURALDOMAIN, serverURL);
    this.adList = adList;
  }
  ...
  public ExportWorkbook createReport() {
    this.addADSheet(this.adList);

    // in order to eliminate double entries in the result list, all necessary entities are added
    // to a HashSet first.
    HashSet<TechnicalComponentRelease> allTCRs = new HashSet<TechnicalComponentRelease>();
    for (ArchitecturalDomain ad : this.adList) {
      allTCRs.addAll(ad.getTechnicalComponentReleases());
    }
