...
/**
 * This class is responsible for creation of an excel report for {@link Product}s.
 */
public class ProductExcelReport extends ExcelReport {
   ...
   * @param products
   *          the {@link Product}s for the report
   * @param queryData
   ...
   */
  public ProductExcelReport(ExportWorkbook context, BuildingBlockTypeDAO bbtDao, AttributeTypeDAO attributeTypeDAO, Set<Product> products,
      ExcelAdditionalQueryData queryData, String serverURL) {
    this.init(context, bbtDao, attributeTypeDAO, queryData, TypeOfBuildingBlock.PRODUCT, serverURL);
  ...
  public ExportWorkbook createReport() {
    ...
    this.addProductSheet(this.products);
    ...
    for (Product product : this.products) {
      ExcelHelper.extractContentsFromBMs(product.getBusinessMappings(), allBPs, allBUs, allISRs, null);
      allBDs.addAll(product.getBusinessDomains());
      allBMs.addAll(product.getBusinessMappings());
