...
/**
 * Concrete sheet importer for responsibility attributes.
 * @author sip
 */
public class ResponsibilitySheetImporter extends AttributeSheetImporter<EnumAttData> {
   ...
   */
  public ResponsibilitySheetImporter(Locale locale, ProcessingLog processingLog) {
  ...
  public void readContentFromSheet(List<Row> sheetContentRows, Map<String, Integer> headline, AttributeData attributeData) {
    ...
    Object userGroupsColumnName = MessageAccess.getStringOrNull(AttributeWorkbook.VALUE_USERGROUP_COLUMN_KEY, getLocale());

    List<ResponsibilityAttData> result = Lists.newArrayList();
    int currentRow = 0;
    while (currentRow < sheetContentRows.size()) {
      ...
      CellValueHolder users = new CellValueHolder(initialRowContent.get(usersColumnName));
      CellValueHolder userGroups = new CellValueHolder(initialRowContent.get(userGroupsColumnName));
      ...
      result.add(new ResponsibilityAttData(name, oldName, description, groupstr, mandatory, activeforbb, multiple, users, userGroups));
    }

    attributeData.setResponsibilityData(result);
  }
}
