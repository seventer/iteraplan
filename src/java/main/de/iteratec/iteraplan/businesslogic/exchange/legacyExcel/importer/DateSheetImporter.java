...
/**
 * Concrete sheet importer for date attributes.
 * @author sip
 */
public class DateSheetImporter extends AttributeSheetImporter<DateAttData> {
   ...
   */
  public DateSheetImporter(Locale locale, ProcessingLog processingLog) {
  ...
  public void readContentFromSheet(List<Row> sheetContentRows, Map<String, Integer> headline, AttributeData attributeData) {
    ...
    List<DateAttData> result = Lists.newArrayList();
    int currentRow = 0;
    while (currentRow < sheetContentRows.size()) {
      ...
      result.add(new DateAttData(name, oldName, description, groupstr, mandatory, activeforbb));
    }

    attributeData.setDateData(result);
  }
