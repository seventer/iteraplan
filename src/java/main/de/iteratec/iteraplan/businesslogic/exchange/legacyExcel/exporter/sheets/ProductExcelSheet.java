...
/**
 * This class represents an excel sheet with contents for {@link Product}s.
 */
public class ProductExcelSheet extends CommonSheetContent<Product> {

  private static final String SHEET_KEY = Constants.BB_PRODUCT_PLURAL;
   ...
   */
  public ProductExcelSheet(Set<Product> contents, ExportWorkbook context, List<AttributeType> activatedAttributeTypes,
  ...
  protected void addSpecificHeaders(List<Header> headers) {
    ...
    String coverageHeaderNameExtended = bm + UNIT_OPENER + process + UNIT_SEPARATOR + bu + UNIT_SEPARATOR + isr + UNIT_CLOSER;
   ...
   */
  @Override
  protected void addSpecificLineContents(Product product, List<Object> lineContents) {
    if (product == null) {
      return;
    }
    lineContents.add(ExcelHelper.concatMultipleHierarchicalNames(product.getBusinessDomains(), IN_LINE_SEPARATOR));
    ...
    ExcelHelper.extractContentsFromBMs(product.getBusinessMappings(), bpSet, buSet, isrSet, null);
    ...
    lineContents.add(ExcelHelper.concatBPandBUandISR(product.getBusinessMappings(), UNIT_OPENER, UNIT_SEPARATOR, UNIT_CLOSER, IN_LINE_SEPARATOR));
