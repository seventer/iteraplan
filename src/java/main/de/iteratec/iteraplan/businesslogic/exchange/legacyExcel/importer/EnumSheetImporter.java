...
/**
 * Concrete sheet importer for enum attributes.
 * @author sip
 */
public class EnumSheetImporter extends AttributeSheetImporter<EnumAttData> {
   ...
   */
  public EnumSheetImporter(Locale locale, ProcessingLog processingLog) {
  ...
  public void readContentFromSheet(List<Row> sheetContentRows, Map<String, Integer> headline, AttributeData attributeData) {
    ...
    List<EnumAttData> result = Lists.newArrayList();
    int currentRow = 0;
    while (currentRow < sheetContentRows.size()) {
      ...
      CellValueHolder multiple = new CellValueHolder(initialRowContent.get(multipleColumnName));
      CellValueHolder activeforbb = new CellValueHolder(initialRowContent.get(activeforbbColumnName));

      List<EnumAttributeValueData> values = Lists.newArrayList();
      EnumAttributeValueData valueData1 = getValueData(valueColumnName, oldvalueColumnName, valueDescriptionColumnName, initialRowContent);
      if (valueData1 != null) {
        values.add(valueData1);
      }
      ...
      if(!isBooleanValue(multiple)) {
        getProcessingLog().warn("[{0}] Mandatory value for attribute {1} doesn\'t match yes/no format.", multiple.getCellRef(), name.getAttributeValue());
      }
      currentRow++;

      for (int j = currentRow; j < sheetContentRows.size(); j++, currentRow++) {
        Row row = sheetContentRows.get(j);
        Map<String, Cell> rowContent = ExcelImportUtilities.readRow(row, headline);
        String tmpName = ExcelImportUtilities.contentAsString(rowContent.get(nameColumnName), getProcessingLog());
        if (StringUtils.isNotBlank(tmpName)) {
          break;
        }

        EnumAttributeValueData valueData = getValueData(valueColumnName, oldvalueColumnName, valueDescriptionColumnName, rowContent);
        if (valueData != null) {
          values.add(valueData);
        }
      }

      result.add(new EnumAttData(name, oldName, description, groupstr, mandatory, activeforbb, multiple, values));
    }

    attributeData.setEnumData(result);
  }

  /**
   * This method creates a EnumAttributeValueData Object. If name is a blank fields, this method returns null.
   * @param valueColumnName
   * @param oldvalueColumnName
   * @param valueDescriptionColumnName
   * @param rowContent
   * @return new EnumAttributeValueData, or null
   */
  private EnumAttributeValueData getValueData(String valueColumnName, String oldvalueColumnName, String valueDescriptionColumnName,
                                              Map<String, Cell> rowContent) {
    Cell attValueCell = rowContent.get(valueColumnName);
    CellValueHolder attValue = new CellValueHolder(attValueCell);
    CellValueHolder attOldValue = new CellValueHolder(rowContent.get(oldvalueColumnName));
    if (StringUtils.isNotBlank(attValue.getAttributeValue())) {
      CellValueHolder attValueDescription = new CellValueHolder(rowContent.get(valueDescriptionColumnName));
      return new EnumAttributeValueData(attValue, attOldValue, attValueDescription);
    }
    else {
      getProcessingLog().error("Skipping value without name in cell {0}.", ExcelImportUtilities.getCellRef(attValueCell));
      return null;
    }
  }
