...
/**
 * This class represents an excel sheet with contents for {@link BusinessFunction}s.
 */
public class BusinessFunctionExcelSheet extends CommonSheetContent<BusinessFunction> {

  private static final String SHEET_KEY = Constants.BB_BUSINESSFUNCTION_PLURAL;
   ...
   */
  public BusinessFunctionExcelSheet(Set<BusinessFunction> contents, ExportWorkbook context, List<AttributeType> activatedAttributeTypes,
   ...
   */
  @Override
  protected void addSpecificLineContents(BusinessFunction bf, List<Object> lineContents) {
    if (bf == null) {
      return;
    }
    lineContents.add(ExcelHelper.concatMultipleHierarchicalNames(bf.getBusinessDomains(), IN_LINE_SEPARATOR));

    lineContents.add(ExcelHelper.concatMultipleHierarchicalNames(bf.getBusinessObjects(), IN_LINE_SEPARATOR));

    lineContents.add(ExcelHelper.concatMultipleHierarchicalNames(bf.getInformationSystems(), IN_LINE_SEPARATOR));
  }
}