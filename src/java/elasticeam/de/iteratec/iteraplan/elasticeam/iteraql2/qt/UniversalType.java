...
/**
 * A query tree element which represents a universal type, possibly holding extensions.
 */
public class UniversalType implements QueryTreeElement {

  private String          identifier;
  private List<Extension> extensions;

  public UniversalType(String identifier, List<Extension> extensions) {
  ...
  public CompilationUnit compile(QueryCompilationContext context) {
    ...
    UniversalTypeExpression type = CompileUtil.findUniversalType(identifier, context.getMetamodel());
    if (type == null) {
      throw new IteraQl2Exception(IteraQl2Exception.NO_SUCH_ELEMENT, "No such universal type: " + identifier);
    }
    CompilationUnit contextUnit = new UniversalTypeCompilationUnit(type);
    ...
    for (Extension extension : extensions) {
      context.setContextCompilationUnit(contextUnit);
      contextUnit = extension.compile(context);
    }
