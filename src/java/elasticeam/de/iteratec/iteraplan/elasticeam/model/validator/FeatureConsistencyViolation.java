...
/**
 * A superclass for all consistency violations related with features.
 */
public abstract class FeatureConsistencyViolation implements ModelConsistencyViolation {

  private FeatureExpression<?> onFeature;

  FeatureConsistencyViolation(FeatureExpression<?> onFeature) {
  ...
  public UniversalTypeExpression getUniversalType() {
    return this.onFeature.getHolder();
  }

  protected boolean isNamedType() {
    if (UniversalTypeExpression.ID_PROPERTY.getPersistentName().equals(onFeature.getPersistentName())) {
      return false;
    }
