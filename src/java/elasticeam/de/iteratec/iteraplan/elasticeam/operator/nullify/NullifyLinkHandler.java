...
public class NullifyLinkHandler extends ADerivedHandler implements LinkHandler {

  public NullifyLinkHandler(Model model) {
    super(model);
  }

  /**{@inheritDoc}**/
  public Collection<LinkExpression> findAll(RelationshipTypeExpression type) {
    RelationshipTypeExpression baseType = ((NullifiedRelationshipType) type).getBaseType();
    List<LinkExpression> allInstances = new ArrayList<LinkExpression>();
    allInstances.addAll(getModel().findAll(baseType));
    allInstances.add(new NullifiedLink(getModel(), (NullifiedRelationshipType) type));
  ...
  public boolean isHandlerFor(RelationshipTypeExpression type) {
    return NullifiedRelationshipType.class.isInstance(type);
  }

  /**{@inheritDoc}**/
  public boolean isHandlerFor(LinkExpression link) {
    return NullifiedLink.class.isInstance(link);
  }
