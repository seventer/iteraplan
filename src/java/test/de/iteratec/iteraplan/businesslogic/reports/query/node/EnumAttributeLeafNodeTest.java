...
/**
 * Test cases for attribute leaf nodes of enumeration attribute types.
 */
public class EnumAttributeLeafNodeTest {

  private static final String NO_ERROR_ERROR_MSG = "An error should have orccured";

  @Test
  public void testNoValueProvided() {
    AttributeLeafNode aln = null;
    Type<InformationSystemRelease> type = InformationSystemReleaseTypeQu.getInstance();

    try {
      aln = EnumAttributeLeafNode.createNode(type, null, 1, Comparator.LIKE, "");
    ...
    try {
      aln = EnumAttributeLeafNode.createNode(type, null, 1, Comparator.NOT_LIKE, "");
    } catch (IteraplanBusinessException ex) {
      assertEquals(NO_ERROR_ERROR_MSG, ex.getErrorCode(), IteraplanErrorMessages.QUERY_NO_VALUE_PROVIDED);
    }
    ...
    if (aln != null) {
      boolean additionalQuery = aln.isAdditionalQueryRequired();
      assertFalse("No additional HQL query should be necessary!", additionalQuery);
    }
  }
