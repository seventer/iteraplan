 ...
 * for {@link de.iteratec.iteraplan.model.TechnicalComponentRelease}s.
 * 
 * @see TechnicalComponentReleaseSheetTest
 */
public class TechnicalComponentReleaseReportTest extends ExcelReportTestBase {

  /**
   * Tests the contents of a report for technical components in Excel 2003 format
   */
  @Test
  public void testTCRReport2003() {
    testReportCreationWithPermissions(TemplateType.EXCEL_2003);
  }

  /**
   * Tests the contents of a report for technical components in Excel 2007 format
   */
  @Test
  public void testTCRReport2007() {
  ...
  private void testReportCreationWithPermissions(TemplateType templateType) {
    ...
    TechnicalComponentReleaseExcelReport report = new TechnicalComponentReleaseExcelReport(context, this.getBbtDAOMock(), this.getAttrTypeDAOMock(), allTCRs,
        null, TEST_SERVER_URL);
    context = report.createReport();

    this.verifySheets(context, 5, DEFAULT_SHEET_KEY, TCR_SHEET_KEY, ISR_SHEET_KEY, AD_SHEET_KEY, IE_SHEET_KEY);

    tofps.remove(TypeOfFunctionalPermission.TECHNICALCOMPONENTRELEASES);
    UserContext.setCurrentUserContext(this.createUserContext(tofps));
    try {
      new TechnicalComponentReleaseExcelReport(context, this.getBbtDAOMock(), this.getAttrTypeDAOMock(), allTCRs, null, TEST_SERVER_URL);
      fail("Created a technical component report without having a permission");
