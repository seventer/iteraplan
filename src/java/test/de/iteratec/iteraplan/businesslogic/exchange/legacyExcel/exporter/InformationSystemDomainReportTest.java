 ...
 * for {@link de.iteratec.iteraplan.model.InformationSystemDomain}s.
 *
 * @see InformationSystemDomainSheetTest
 */
public class InformationSystemDomainReportTest extends ExcelReportTestBase {

  /**
   * Tests the contents of a report for information system in Excel 2003 format
   */
  @Test
  public void testISDReport2003() {
    testReportCreationWithPermissions(TemplateType.EXCEL_2003);
  }

  /**
   * Tests the contents of a report for information system in Excel 2007 format
   */
  @Test
  public void testISDReport2007() {
  ...
  private void testReportCreationWithPermissions(TemplateType templateType) {
    ...
    Set<InformationSystemDomain> allISDs = new HashSet<InformationSystemDomain>();

    InformationSystemDomainExcelReport report = new InformationSystemDomainExcelReport(context, this.getBbtDAOMock(), this.getAttrTypeDAOMock(), allISDs, null,
        TEST_SERVER_URL);
    context = report.createReport();

    this.verifySheets(context, 3, DEFAULT_SHEET_KEY, ISD_SHEET_KEY, ISR_SHEET_KEY);

    tofps.remove(TypeOfFunctionalPermission.INFORMATIONSYSTEMDOMAIN);
    UserContext.setCurrentUserContext(this.createUserContext(tofps));
    try {
      new InformationSystemDomainExcelReport(context, this.getBbtDAOMock(), this.getAttrTypeDAOMock(), allISDs, null, TEST_SERVER_URL);
      fail("Created a information system domain report without having a permission");
