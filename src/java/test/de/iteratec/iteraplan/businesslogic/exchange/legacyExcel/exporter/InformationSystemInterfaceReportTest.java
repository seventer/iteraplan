 ...
 * for {@link de.iteratec.iteraplan.model.InformationSystemInterface}s.
 * 
 * @see InformationSystemInterfaceSheetTest
 */
public class InformationSystemInterfaceReportTest extends ExcelReportTestBase {

  /**
   * Tests the contents of a report for information system interfaces in Excel 2003 format
   */
  @Test
  public void testISIReport2003() {
  ...
  private void testReportCreationWithPermissions(TemplateType templateType) {
    ...
    InformationSystemInterfaceExcelReport report = new InformationSystemInterfaceExcelReport(context, this.getBbtDAOMock(), this.getAttrTypeDAOMock(), allISIs,
        null, TEST_SERVER_URL);
    context = report.createReport();

    this.verifySheets(context, 4, DEFAULT_SHEET_KEY, ISI_SHEET_KEY, BO_SHEET_KEY, TCR_SHEET_KEY);

    tofps.remove(TypeOfFunctionalPermission.INFORMATIONSYSTEMINTERFACE);
    UserContext.setCurrentUserContext(this.createUserContext(tofps));
    try {
      new InformationSystemInterfaceExcelReport(context, this.getBbtDAOMock(), this.getAttrTypeDAOMock(), allISIs, null, TEST_SERVER_URL);
      fail("Created an information system interface report without having a permission");
