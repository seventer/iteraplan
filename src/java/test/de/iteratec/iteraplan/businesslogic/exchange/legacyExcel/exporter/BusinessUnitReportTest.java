 ...
 * for {@link de.iteratec.iteraplan.model.BusinessUnit}s.
 * 
 * @see BusinessUnitSheetTest
 */
public class BusinessUnitReportTest extends ExcelReportTestBase {

  /**
   * Tests the contents of a report for business units in Excel 2003 format
   */
  @Test
  public void testBUReport2003() {
    testReportCreationWithPermissions(TemplateType.EXCEL_2003);
  }

  /**
   * Tests the contents of a report for business units in Excel 2007 format
   */
  @Test
  public void testBUReport2007() {
  ...
  private void testReportCreationWithPermissions(TemplateType templateType) {
    ...
    BusinessUnitExcelReport report = new BusinessUnitExcelReport(context, this.getBbtDAOMock(), this.getAttrTypeDAOMock(), allBUs, null, TEST_SERVER_URL);
    context = report.createReport();

    this.verifySheets(context, 7, DEFAULT_SHEET_KEY, BU_SHEET_KEY, BD_SHEET_KEY, BP_SHEET_KEY, PRODUCT_SHEET_KEY, BM_SHEET_KEY, ISR_SHEET_KEY);

    tofps.remove(TypeOfFunctionalPermission.BUSINESSUNIT);
    UserContext.setCurrentUserContext(this.createUserContext(tofps));
    try {
      new BusinessUnitExcelReport(context, this.getBbtDAOMock(), this.getAttrTypeDAOMock(), allBUs, null, TEST_SERVER_URL);
      fail("Created a business unit report without having a permission");
