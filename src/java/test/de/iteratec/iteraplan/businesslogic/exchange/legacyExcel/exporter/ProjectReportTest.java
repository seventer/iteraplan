 ...
 * for {@link de.iteratec.iteraplan.model.Project}s.
 * 
 * @see ProjectSheetTest
 */
public class ProjectReportTest extends ExcelReportTestBase {

  /**
   * Tests the contents of a report for projects in Excel 2003 format
   */
  @Test
  public void testProjectReport2003() {
    testReportCreationWithPermissions(TemplateType.EXCEL_2003);
  }

  /**
   * Tests the contents of a report for projects in Excel 2007 format
   */
  @Test
  public void testProjectReport2007() {
  ...
  private void testReportCreationWithPermissions(TemplateType templateType) {
    ...
    ProjectExcelReport report = new ProjectExcelReport(context, this.getBbtDAOMock(), this.getAttrTypeDAOMock(), allProjects, null, TEST_SERVER_URL);
    context = report.createReport();

    this.verifySheets(context, 3, DEFAULT_SHEET_KEY, PROJECT_SHEET_KEY, ISR_SHEET_KEY);

    tofps.remove(TypeOfFunctionalPermission.PROJECT);
    UserContext.setCurrentUserContext(this.createUserContext(tofps));
    try {
      new ProjectExcelReport(context, this.getBbtDAOMock(), this.getAttrTypeDAOMock(), allProjects, null, TEST_SERVER_URL);
      fail("Created a project report without having a permission");
