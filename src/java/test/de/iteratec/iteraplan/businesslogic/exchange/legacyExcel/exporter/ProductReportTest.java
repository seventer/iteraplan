 ...
 * for {@link de.iteratec.iteraplan.model.Product}s.
 * 
 * @see ProductSheetTest
 */
public class ProductReportTest extends ExcelReportTestBase {

  /**
   * Tests the contents of a report for products in Excel 2003 format
   */
  @Test
  public void testProductReport2003() {
    testReportCreationWithPermissions(TemplateType.EXCEL_2003);
  }

  /**
   * Tests the contents of a report for products in Excel 2007 format
   */
  @Test
  public void testProductReport2007() {
  ...
  private void testReportCreationWithPermissions(TemplateType templateType) {
    ...
    ProductExcelReport report = new ProductExcelReport(context, this.getBbtDAOMock(), this.getAttrTypeDAOMock(), allProducts, null, TEST_SERVER_URL);
    context = report.createReport();

    this.verifySheets(context, 7, DEFAULT_SHEET_KEY, PRODUCT_SHEET_KEY, BD_SHEET_KEY, BP_SHEET_KEY, BU_SHEET_KEY, BM_SHEET_KEY, ISR_SHEET_KEY);

    tofps.remove(TypeOfFunctionalPermission.PRODUCT);
    UserContext.setCurrentUserContext(this.createUserContext(tofps));
    try {
      new ProductExcelReport(context, this.getBbtDAOMock(), this.getAttrTypeDAOMock(), allProducts, null, TEST_SERVER_URL);
      fail("Created a product report without having a permission");
