...
public class SvgMasterplanSpecificationTest extends BaseTransactionalTestSupport {
  ...
  private MasterplanOptionsBean initOptions() {
    ...
    options.setSelectedGraphicFormat(Constants.REPORTS_EXPORT_GRAPHICAL_SVG);
  ...
  private boolean writeDiagramToFile(MasterplanOptionsBean masterplanOptions, List<BuildingBlock> selectedLevel0Bbs, String fileName) {
    ...
    SvgMasterplanExport svgExport = new SvgMasterplanExport(diagram, masterplanOptions, attributeTypeService, attributeValueService);
    Document document = svgExport.createDiagram();

    File destinationFile = getDestinationFile(fileName, masterplanOptions.getSelectedGraphicFormat());
    try {
      FileOutputStream outputStream = new FileOutputStream(destinationFile);
      if (Constants.REPORTS_EXPORT_GRAPHICAL_SVG.equals(masterplanOptions.getSelectedGraphicFormat())) {
        SvgGraphicWriter.writeToSvg(document, outputStream);
      }
      else if (Constants.REPORTS_EXPORT_GRAPHICAL_JPEG.equals(masterplanOptions.getSelectedGraphicFormat())) {
        SvgGraphicWriter.writeToJPEG(SvgGraphicWriter.transformToDomDocument(document), outputStream);
      }
      else if (Constants.REPORTS_EXPORT_GRAPHICAL_PDF.equals(masterplanOptions.getSelectedGraphicFormat())) {
        SvgGraphicWriter.writeToPDF(SvgGraphicWriter.transformToDomDocument(document), outputStream);
      }
      else if (Constants.REPORTS_EXPORT_GRAPHICAL_PNG.equals(masterplanOptions.getSelectedGraphicFormat())) {
        SvgGraphicWriter.writeToPNG(SvgGraphicWriter.transformToDomDocument(document), outputStream);
      }
      else {
        try {
          outputStream.close();
  ...
  private File getDestinationFile(String fileName, String exportFormat) {
    String extension = "";
    if (Constants.REPORTS_EXPORT_GRAPHICAL_SVG.equals(exportFormat)) {
      extension = ".svg";
    }
    else if (Constants.REPORTS_EXPORT_GRAPHICAL_JPEG.equals(exportFormat)) {
      extension = ".jpg";
    }
    else if (Constants.REPORTS_EXPORT_GRAPHICAL_PDF.equals(exportFormat)) {
      extension = ".pdf";
    }
    else if (Constants.REPORTS_EXPORT_GRAPHICAL_PNG.equals(exportFormat)) {
      extension = ".png";
    }
