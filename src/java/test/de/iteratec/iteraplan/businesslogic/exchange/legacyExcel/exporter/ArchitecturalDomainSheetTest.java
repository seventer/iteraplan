 ...
 * export for {@link de.iteratec.iteraplan.model.ArchitecturalDomain}s.
 * 
 * @see ArchitecturalDomainReportTest
 */
public class ArchitecturalDomainSheetTest extends ExcelSheetTestBase {

  /**
   * Tests the content of the excel sheet export for {@link ArchitecturalDomain}s in Excel 2003
   */
  @Test
  public void testADSheet2003() {
  ...
  /**
  * Tests the content of the excel sheet export for {@link ArchitecturalDomain}s in Excel 2007
  */
  @Test
  public void testADSheet2007() {
  ...
  private void createAndVerifySheet(ExportWorkbook context) {
    ...
    Set<ArchitecturalDomain> adSet = createTestArchitecturalDomains(exporterDataUtil, subscribedUsers, "testAD", 10);

    BuildingBlockType bbt = new BuildingBlockType(TypeOfBuildingBlock.ARCHITECTURALDOMAIN);
    bbt.setId(Integer.valueOf(0));

    ArchitecturalDomainExcelSheet sheet = new ArchitecturalDomainExcelSheet(adSet, context, new ArrayList<AttributeType>(), null, bbt, TEST_URL);
    sheet.createSheet();
    ...
    List<ArchitecturalDomain> sortedADs = ExcelHelper.sortEntities(adSet, new IdentityStringComparator());

    assertSame(Integer.valueOf(context.getNumberOfSheets()), Integer.valueOf(2));
    Sheet resultSheet = context.getSheetAt(1);
    
    this.verifyRowContent(this.retrieveNextRow(resultSheet), MessageAccess.getStringOrNull(ExcelReportTestBase.AD_SHEET_KEY, context.getLocale()));

    List<String> expContent = exporterDataUtil.initCommonExpectedHeaders(context.getLocale(), ExcelReportTestBase.AD_SHEET_KEY);
    ...
    for (ArchitecturalDomain entity : sortedADs) {
      expContent = createAdExpRowContent(entity);
      this.verifyRowContent(this.retrieveNextRow(resultSheet), expContent.toArray());
    }
  }

  private <T> List<String> createAdExpRowContent(ArchitecturalDomain entity) {
    List<String> expContent = createBasicHierarchicalEntityContent("HYPERLINK(\"testURL/show/architecturaldomain/%d\",%d)", entity);
  ...
  private Set<ArchitecturalDomain> createTestArchitecturalDomains(ExporterDataUtil exporterDataUtil, Set<User> subscribedUsers, String name,
                                                                  int entitiesNumber) {
    Set<ArchitecturalDomain> entities = new HashSet<ArchitecturalDomain>();
    for (int i = 0; i < entitiesNumber; i++) {
      ...
      entity.addTechnicalComponentReleases(exporterDataUtil.createTCRs(i + 1, name + " - TechnicalComponentRelease", 5));
