 ...
 * for {@link de.iteratec.iteraplan.model.BusinessDomain}s.
 * 
 * @see BusinessDomainSheetTest
 */
public class BusinessDomainReportTest extends ExcelReportTestBase {

  /**
   * Tests the contents of a report for business domains in Excel 2003 format
   */
  @Test
  public void testBDReport2003() {
    testReportCreationWithPermissions(TemplateType.EXCEL_2003);
  }

  /**
   * Tests the contents of a report for business domains in Excel 2007 format
   */
  @Test
  public void testBDReport2007() {
  ...
  private void testReportCreationWithPermissions(TemplateType templateType) {
    ...
    BusinessDomainExcelReport report = new BusinessDomainExcelReport(context, this.getBbtDAOMock(), this.getAttrTypeDAOMock(), allBDs, null,
        TEST_SERVER_URL);
    context = report.createReport();

    this.verifySheets(context, 7, DEFAULT_SHEET_KEY, BD_SHEET_KEY, BP_SHEET_KEY, BF_SHEET_KEY, PRODUCT_SHEET_KEY, BU_SHEET_KEY, BO_SHEET_KEY);

    tofps.remove(TypeOfFunctionalPermission.BUSINESSDOMAIN);
    UserContext.setCurrentUserContext(this.createUserContext(tofps));
    try {
      new BusinessDomainExcelReport(context, this.getBbtDAOMock(), this.getAttrTypeDAOMock(), allBDs, null, TEST_SERVER_URL);
      fail("Created a business domain report without having a permission");
