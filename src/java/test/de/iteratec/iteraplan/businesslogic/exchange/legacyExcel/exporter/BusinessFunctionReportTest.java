 ...
 * for {@link de.iteratec.iteraplan.model.BusinessFunction}s.
 * 
 * @see BusinessFunctionSheetTest
 */
public class BusinessFunctionReportTest extends ExcelReportTestBase {

  /**
   * Tests the contents of a report for business functions in Excel 2003 format
   */
  @Test
  public void testBFReport2003() {
    testReportCreationWithPermissions(TemplateType.EXCEL_2003);
  }

  /**
   * Tests the contents of a report for business functions in Excel 2007 format
   */
  @Test
  public void testBFReport2007() {
  ...
  private void testReportCreationWithPermissions(TemplateType templateType) {
    ...
    Set<BusinessFunction> allBFs = new HashSet<BusinessFunction>();

    BusinessFunctionExcelReport report = new BusinessFunctionExcelReport(context, this.getBbtDAOMock(), this.getAttrTypeDAOMock(), allBFs, null,
        TEST_SERVER_URL);
    context = report.createReport();

    this.verifySheets(context, 5, DEFAULT_SHEET_KEY, BF_SHEET_KEY, BD_SHEET_KEY, BO_SHEET_KEY, ISR_SHEET_KEY);

    tofps.remove(TypeOfFunctionalPermission.BUSINESSFUNCTION);
    UserContext.setCurrentUserContext(this.createUserContext(tofps));
    try {
      new BusinessFunctionExcelReport(context, this.getBbtDAOMock(), this.getAttrTypeDAOMock(), allBFs, null, TEST_SERVER_URL);
      fail("Created a business function report without having a permission");
