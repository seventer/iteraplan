 ...
 * export for {@link de.iteratec.iteraplan.model.InfrastructureElement}s.
 * 
 * @see InfrastructureElementReportTest
 */
public class InfrastructureElementSheetTest extends ExcelSheetTestBase {

  /**
   * Tests the content of the excel sheet export for {@link InfrastructureElement}s in Excel 2003
   */
  @Test
  public void testIESheet2003() {
  ...
  /**
  * Tests the content of the excel sheet export for {@link InfrastructureElement}s in Excel 2007
  */
  @Test
  public void testIESheet2007() {
  ...
  private void createAndVerifySheet(ExportWorkbook context) {
    ...
    Set<InfrastructureElement> ieSet = createTestInfrastructureElements(exporterDataUtil, subscribedUsers, "testIE", 10);

    BuildingBlockType bbt = new BuildingBlockType(TypeOfBuildingBlock.INFRASTRUCTUREELEMENT);
    bbt.setId(Integer.valueOf(0));

    InfrastructureElementExcelSheet sheet = new InfrastructureElementExcelSheet(ieSet, context, new ArrayList<AttributeType>(), null, bbt, TEST_URL);
    sheet.createSheet();
    ...
    List<InfrastructureElement> sortedIEs = ExcelHelper.sortEntities(ieSet, new IdentityStringComparator());

    assertSame(Integer.valueOf(context.getNumberOfSheets()), Integer.valueOf(2));
    Sheet resultSheet = context.getSheetAt(1);

    this.verifyRowContent(this.retrieveNextRow(resultSheet), MessageAccess.getStringOrNull(ExcelReportTestBase.IE_SHEET_KEY, context.getLocale()));
    
    List<String> expContent = exporterDataUtil.initCommonExpectedHeaders(context.getLocale(), ExcelReportTestBase.IE_SHEET_KEY);
    
    expContent.add(3, MessageAccess.getStringOrNull(Constants.ASSOC_USEDBY, context.getLocale()));
    expContent.add(3, MessageAccess.getStringOrNull(Constants.ASSOC_USES, context.getLocale()));
   
    expContent.add(MessageAccess.getStringOrNull(ISR_PLURAL_MESSAGE_KEY, context.getLocale()));
    expContent.add(MessageAccess.getStringOrNull("technicalRealisation", context.getLocale()));

    this.verifyRowContent(this.retrieveNextRow(resultSheet), expContent.toArray());

    for (InfrastructureElement entity : sortedIEs) {
      expContent = createIeExpRowContent(entity);
      this.verifyRowContent(this.retrieveNextRow(resultSheet), expContent.toArray());
    }
  }

  private List<String> createIeExpRowContent(InfrastructureElement entity) {
    List<String> expContent = createBasicHierarchicalEntityContent("HYPERLINK(\"testURL/show/infrastructureelement/%d\",%d)", entity);
    
    expContent.add(3, ExcelHelper.concatMultipleHierarchicalNames(entity.getParentComponents(), ExcelSheet.IN_LINE_SEPARATOR));
    expContent.add(3, ExcelHelper.concatMultipleHierarchicalNames(entity.getBaseComponents(), ExcelSheet.IN_LINE_SEPARATOR));
  ...
  private Set<InfrastructureElement> createTestInfrastructureElements(ExporterDataUtil exporterDataUtil, Set<User> subscribedUsers, String name,
                                                                      int entitiesNumber) {
    ...
    for (int i = 0; i < entitiesNumber; i++) {
      ...
      Set<TechnicalComponentRelease> tcrs = exporterDataUtil.createTCRs(i + 1, name + " - TechnicalComponentRelease", new int[] { 5 });
