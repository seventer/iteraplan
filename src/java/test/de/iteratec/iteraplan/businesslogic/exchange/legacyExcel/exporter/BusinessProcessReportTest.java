 ...
 * for {@link de.iteratec.iteraplan.model.BusinessProcess}s.
 * 
 * @see BusinessProcessSheetTest
 */
public class BusinessProcessReportTest extends ExcelReportTestBase {

  /**
   * Tests the contents of a report for business processes in Excel 2003 format
   */
  @Test
  public void testBPReport2003() {
    testReportCreationWithPermissions(TemplateType.EXCEL_2003);
  }

  /**
   * Tests the contents of a report for business processes in Excel 2007 format
   */
  @Test
  public void testBPReport2007() {
  ...
  private void testReportCreationWithPermissions(TemplateType templateType) {
    ...
    BusinessProcessExcelReport report = new BusinessProcessExcelReport(context, this.getBbtDAOMock(), this.getAttrTypeDAOMock(), allBPs, null, TEST_SERVER_URL);
    context = report.createReport();

    this.verifySheets(context, 7, DEFAULT_SHEET_KEY, BP_SHEET_KEY, BD_SHEET_KEY, PRODUCT_SHEET_KEY, BU_SHEET_KEY, BM_SHEET_KEY, ISR_SHEET_KEY);

    tofps.remove(TypeOfFunctionalPermission.BUSINESSPROCESS);
    UserContext.setCurrentUserContext(this.createUserContext(tofps));
    try {
      new BusinessProcessExcelReport(context, this.getBbtDAOMock(), this.getAttrTypeDAOMock(), allBPs, null, TEST_SERVER_URL);
      fail("Created a business process report without having a permission");
