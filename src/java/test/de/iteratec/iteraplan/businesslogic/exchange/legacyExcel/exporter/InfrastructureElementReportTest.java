 ...
 * for {@link de.iteratec.iteraplan.model.InfrastructureElement}s.
 * 
 * @see InfrastructureElementSheetTest
 */
public class InfrastructureElementReportTest extends ExcelReportTestBase {

  /**
   * Tests the contents of a report for infrastructure elements in Excel 2003 format
   */
  @Test
  public void testIEReport2003() {
    testReportCreationWithPermissions(TemplateType.EXCEL_2003);
  }

  /**
   * Tests the contents of a report for infrastructure elements in Excel 2007 format
   */
  @Test
  public void testIEReport2007() {
  ...
  private void testReportCreationWithPermissions(TemplateType templateType) {
    ...
    InfrastructureElementExcelReport report = new InfrastructureElementExcelReport(context, this.getBbtDAOMock(), this.getAttrTypeDAOMock(), allIEs, null,
        TEST_SERVER_URL);
    context = report.createReport();

    this.verifySheets(context, 4, DEFAULT_SHEET_KEY, IE_SHEET_KEY, ISR_SHEET_KEY, TCR_SHEET_KEY);

    tofps.remove(TypeOfFunctionalPermission.INFRASTRUCTUREELEMENT);
    UserContext.setCurrentUserContext(this.createUserContext(tofps));
    try {
      new InfrastructureElementExcelReport(context, this.getBbtDAOMock(), this.getAttrTypeDAOMock(), allIEs, null, TEST_SERVER_URL);
      fail("Created an infrastructure element report without having a permission");
