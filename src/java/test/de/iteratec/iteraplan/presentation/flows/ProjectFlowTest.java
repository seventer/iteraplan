...
import de.iteratec.iteraplan.presentation.dialog.Project.ProjectMemBean;
import de.iteratec.iteraplan.presentation.dialog.common.ComponentMode;


/**
 * JUnit Test of the project flow.
 * 
 * @author dzu
 */
public class ProjectFlowTest extends AbstractFlowTestBase {

  private BuildingBlockFrontendService<ProjectMemBean> projectFrontendService;
  ...
  protected void setUp() {
    ...
    projectFrontendService = EasyMock.createNiceMock(BuildingBlockFrontendService.class);
    EasyMock.expect(projectFrontendService.getEntityName()).andReturn("project");
    EasyMock.expect(projectFrontendService.getFlowLabel((String) EasyMock.anyObject())).andReturn("");

    ProjectMemBean memBean = new ProjectMemBean();
    memBean.setComponentModel(new ProjectComponentModel(ComponentMode.READ));
    EasyMock.expect(projectFrontendService.getMemBean(Integer.valueOf(1))).andReturn(memBean);
  }

  @Override
  protected FlowDefinitionResource getResource(FlowDefinitionResourceFactory resourceFactory) {
    return resourceFactory.createFileResource("WebContent/WEB-INF/flow/projectFlow.xml");
  }

  @Override
  protected void configureFlowBuilderContext(MockFlowBuilderContext builderContext) {
    super.configureFlowBuilderContext(builderContext);
    builderContext.registerBean("projectFrontendService", projectFrontendService);
    builderContext.registerBean("historyService", historyService);
  }

  @Test
  public void testStartProjectFlow() {
    EasyMock.replay(projectFrontendService, historyService);
  ...
  @Test
  public void testCreateProjectFlow() {
    EasyMock.replay(projectFrontendService, historyService);
