...
/**
 *  This class is responsible for testing the InformationSystemController. It ensures that the JSON/REST API works properly.
 */
public class InformationSystemControllerTest {
  private InformationSystemController controller;
  private JsonApiTestHelper           testHelper;

  @Test
  public void testList() {
    testHelper = new JsonApiTestHelper(false);
    controller = testHelper.getInformationSystemController();

    MockHttpServletRequest req = new MockHttpServletRequest();
    MockHttpServletResponse res = new MockHttpServletResponse();
    try {
      controller.list(Boolean.FALSE, false, req, res);
  ...
  private void validateListItem(Map<String, Object> item, int number) {
    ...
    Integer expectedId = testHelper.getInformationSystemReleaseId(number);
    Integer actualId = Integer.valueOf(Integer.parseInt((String) item.get("id")));
    Assert.assertEquals(expectedId, actualId);

    Assert.assertTrue(item.containsKey("name"));
    String name = testHelper.getInformationSystemNameWithVersion(number);
    Assert.assertTrue(((String) item.get("name")).endsWith(name));

    Assert.assertTrue(item.containsKey("elementUri"));
    Assert.assertTrue(((String) item.get("elementUri")).endsWith("/informationsystem/json/" + item.get("id").toString()));
  ...
  public void testDetailsISR0() {
    ...
    MockHttpServletResponse res = new MockHttpServletResponse();
    try {
      controller.details(req, res, testHelper.getInformationSystemReleaseId(0));
    } catch (IOException ex) {
      //ignore
    }
    ...
    Assert.assertFalse(responseContent.containsKey("parentUri"));

    Assert.assertTrue(responseContent.containsKey("connectedBusinessUnits"));

    @SuppressWarnings("unchecked")
    List<Object> cbu = (List<Object>) responseContent.get("connectedBusinessUnits");

    Assert.assertTrue(cbu.size() == 2);

    String bu0 = (String) cbu.get(0);
    String bu1 = (String) cbu.get(1);

    Assert.assertTrue(bu0.contains(testHelper.getBusinessUnitId(0).toString()));
    Assert.assertTrue(bu1.contains(testHelper.getBusinessUnitId(1).toString()));

    Assert.assertTrue(responseContent.keySet().size() == 8);
  ...
  public void testDetailsISR1() {
    ...
    MockHttpServletResponse res = new MockHttpServletResponse();
    try {
      controller.details(req, res, testHelper.getInformationSystemReleaseId(1));
    } catch (IOException ex) {
      //ignore
    }
    ...
    Assert.assertTrue(parentUri.endsWith("/informationsystem/json/" + testHelper.getInformationSystemReleaseId(0).toString()));
    ...
    String bu = (String) cbu.get(0);

    Assert.assertTrue(responseContent.keySet().size() == 9);

    Assert.assertTrue(bu.contains(testHelper.getBusinessUnitId(1).toString()));
  ...
  public void testDetailsISR2() {
    ...
    MockHttpServletResponse res = new MockHttpServletResponse();
    try {
      controller.details(req, res, testHelper.getInformationSystemReleaseId(2));
    } catch (IOException ex) {
      //ignore
    }
    ...
    Assert.assertFalse(responseContent.containsKey("connectedBusinessUnits"));
  ...
  private void validateDetailResponseKeys(Map<String, Object> responseContent, int number) {
    ...
    Assert.assertEquals(testHelper.getInformationSystemName(number), responseContent.get("name"));

    Assert.assertTrue(responseContent.containsKey("id"));
    Assert.assertEquals(testHelper.getInformationSystemReleaseId(number), responseContent.get("id"));
    ...
    Assert.assertTrue(responseContent.containsKey("version"));
    Assert.assertEquals(testHelper.getInformationSystemReleaseVersion(number), responseContent.get("version"));

    Assert.assertTrue(responseContent.containsKey("status"));
    Assert.assertEquals("Current", responseContent.get("status"));
