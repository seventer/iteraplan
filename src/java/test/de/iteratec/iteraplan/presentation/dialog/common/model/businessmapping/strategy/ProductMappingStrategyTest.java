 ...
 *
 */
public class ProductMappingStrategyTest {

  private final BusinessMappingStrategy mappingStrategy = BusinessMappingStrategyFactory.getStrategyFor(Product.class);
  ...
  public void testCreateBusinessMappings() {
    ...
    final BusinessProcess bp = new BusinessProcess();

    final List<InformationSystemRelease> isrs = Lists.newArrayList(isr1, isr2);
    final List<BusinessProcess> bps = Lists.newArrayList(bp);
    final List<BusinessUnit> bus = Lists.newArrayList(bu1);
    ...
    Assert.assertEquals(bp, businessMapping1.getBusinessProcess());

    Assert.assertEquals(isr2, businessMapping2.getInformationSystemRelease());
    Assert.assertEquals(bu1, businessMapping2.getBusinessUnit());
    Assert.assertEquals(bp, businessMapping2.getBusinessProcess());
  }
  ...
  public void testCreateBusinessMappingsWithTopLevelNames() {
    ...
    final BusinessProcess bp = new BusinessProcess();
    bp.setName(BusinessProcess.TOP_LEVEL_NAME);
  ...
  @Test
  public void testCreateBusinessMappingsWithTopLevelNamesAndISR() {
    final InformationSystemRelease isr1 = new InformationSystemRelease();
    final InformationSystemRelease isr2 = new InformationSystemRelease();
    final BusinessUnit bu1 = new BusinessUnit();
    bu1.setName(BusinessUnit.TOP_LEVEL_NAME);
  ...
  public void testAddOwningEntity() {
    ...
    Assert.assertEquals(product, businessMapping.getProduct());
    Assert.assertThat(product.getBusinessMappings(), JUnitMatchers.hasItem(businessMapping));
  }
  ...
  public void testAddOwningEntityForInvalidBB() {
    final BusinessMapping businessMapping = new BusinessMapping();
    final BusinessProcess bp = new BusinessProcess();
    mappingStrategy.addOwningEntity(businessMapping, bp);
  }
  ...
  public void testDoesMappingExist() {
    ...
    bmToCheck.setBusinessUnit(bu1);
    bmToCheck.setBusinessProcess(bp);
