	...
	<c:when test="${functionalPermission == true}">
		<div id="SupportingQueryModul" class="row-fluid module">
			<div class="module-heading">
				<fmt:message key="permission.query.header" />
			</div>
			<div class="row-fluid">
						...
						<table class="searchResultView table table-striped table-condensed tableInModule">
							...
							<thead>
								<tr>
									...
									<th colspan="2">
										<fmt:message key="permission.query.query" />
									</th>
								</tr>
							</thead>
							<tbody>
								...
								<c:forEach items="${dialogMemory.configurations}" var="configuration" varStatus="loopStatus">
									...
									<tr>
										...
										<td>
											<input type="submit" value="<fmt:message key="button.run" />" class="btn" onclick="createHiddenField('selectedPermissionQuery', '<c:out value="${configuration.name}"/>');createHiddenField('selectedPermissionQueryNumber', '<c:out value="${index}"/>');" />
										</td>
									</tr>
								</c:forEach>
							</tbody>
